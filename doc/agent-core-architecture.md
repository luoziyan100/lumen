...
~/.lumen/workspaces/<project_id>/
  sessions/<task_id>/   # ← 会话独立工作区(owner 拍板 2026-07-05):agent 读写、上传、
    papers/             #    资产列表全部以此为根;会话之间互不可见
    notes/  drafts/  docs/  images/  uploads/  scratch/
  (项目根保留旧数据与兼容语义;项目级共享/沉淀区留给 Projects 阶段 2 决策)
  .index.sqlite         # Evidence Index：工作区产物之上的结构化索引(未接线)
```

> 修订(2026-07-05):工作区从项目级共享改为**会话独立**——owner 权衡"积累 vs 会话互扰"后拍板
> 后者优先;跨会话共享改由 Projects 功能的项目级共享区(阶段 2,见 briefs/active/projects.md)按需恢复。
> 实现:`makeWorkspace(projectId, taskId?)`,带 taskId 定根 sessions/<taskId>,不带保持旧语义(兼容/旧数据)。
...
| **M7** | Tauri 薄客户端 | ✅ **v0 已可双击**（2026-07-05）：本机编译打包出 Lumen.app + dmg（aarch64）。壳的三职责落地：探活 portfile →（无活服务才）拉起 node sidecar → 读 {port,token} 经 initialization_script 注入前端（打包态无 vite 注入插件，这是过 WS 鉴权唯一的门）→ 退出杀自拉的 sidecar；CSP 放行 127.0.0.1 的 ws/http 与 data: 图。真机验证：app 连上已运行服务、带 token WS 握手取回真实任务列表。**M7.1 待办**：bundle node+service（分发级，注意 better-sqlite3 ABI）、关窗留 dock 续跑（tray/Reopen，当前关窗即退）、字体本地化 |
...
| **M10** | 工具层加固（2026-06-10，benchmark 对比 OpenClaw 暴露真实质量缺口）：① **grep path 指向文件时 walkFiles 抛 ENOTDIR 静默返回空** → 修复（这是 Soul Computing 任务三次 grep 全"无匹配"、漏掉论文核心挑战、退化为复述 intro 的根因）② grep 命中带字符偏移 charOffset + 长行截断（PDF 抽取常整段无换行，line 号无意义）③ read_file 加 offset/limit 分段读（旧实现截断时提示"分段读取"却根本没给参数）④ extract_pdf 长文存盘后引导用 grep/read_file 读全文，不只凭 20K 预览作答。**输出质量的提示词层（研究判断/批判距离/输出契约）单列，待与 owner 共定** | ✅ 工具层完成 |

> 测试现状：`packages/agent-service` **90/90 通过**，零 mock 充数（替身只在 ModelPort / HttpClient / fetch 网络缝）。
...
> **系统提示词重写（研究判断 vs 复述、对宏大叙事的批判距离、"X 是什么/深读"的输出契约）——已与 owner 确认方向，写法待专门讨论**。
