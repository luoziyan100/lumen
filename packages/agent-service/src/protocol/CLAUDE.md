...
职责:agent-service 与 UI 之间的单 WebSocket 协议:UI 发 submit/continue/subscribe/cancel/archive_task/answer_user/list/list_projects/create_project/设置类消息,service 推事件流(event/tasks/projects/assets/settings/ok/error)。事件流是 task_events 的实时投影;断线靠 subscribe 回放拉齐。`answer_user` 解开 `ask_user` 挂起(见 `doc/ask-user.md`)。
