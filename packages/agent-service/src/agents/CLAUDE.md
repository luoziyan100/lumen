...
- `persona.ts` — LUMEN_PERSONA:人格剧本(L0–L3)+ 工具/可视化/计划/问用户/Skills/Sources 能力段。人格源自 briefs 工程线
  ...
  歧义决策→`ask_user`(见 `doc/ask-user.md`);可运行工作流→`run_skill`;新包落盘后→`install_skill`(≠ memory);
...
- 能力合同段(工具用法 / show-widget / todo_write / ask_user / run_skill / install_skill / Sources 格式)可随对应 brief 追加,不强制走人格回测。
